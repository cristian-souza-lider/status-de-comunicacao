...
def processar_planilhas_consolidadas():
    ...
    # Declaração explícita no início do escopo para evitar qualquer erro de definição
    pasta_pendente = None
    ...
    # 1. Varre todas as subpastas de horas de TODOS os dias do mês atual (ex: 2026\06\*\*h)
    ...
    for pasta in subpastas_hora:
        ...
        if nome_pasta[:-1].isdigit():
            ...
            if not os.path.exists(marcador):
                # E se já possuir pelo menos os 1 arquivos completos carregados pelo Power Automate
